...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check = "SELECT * FROM phancong WHERE MaCT = '$MaCT'";
    $checkkq = mysqli_query($conn, $check);

    if (mysqli_num_rows($checkkq) > 0) {
        // Nếu đã có phân công, thực hiện UPDATE
        $updatesql = "UPDATE phancong SET MaGV = '$MaGV', Trangthai = '$Trangthai'  WHERE MaCT = '$MaCT'";
            ...
            if ($Trangthai == 'Đã duyệt') {
                ...
                try {
                    ...
                    $mail->Body    = "Xin chào {$gv_info['TenGV']},<br><br>"
                                    ...
                                    . "- Số lượng bài: {$chamthi_info['SLbai']}<br>"
                                    . "- Buổi: {$chamthi_info['Buoi']}<br><br>"
        ...
        if ($insertkq) {
            ...
            if ($Trangthai== 'Đã duyệt') {
                ...
                try {
                    ...
                    $mail->isHTML(true);
                    $mail->CharSet = 'UTF-8';
                    $mail->Subject = 'Thông báo Lịch chấm thi bạn đăng ký đã được duyệt';
